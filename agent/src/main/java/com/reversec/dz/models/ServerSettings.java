package com.reversec.dz.models;
...
public class ServerSettings implements OnSharedPreferenceChangeListener {
	...
	private String getKeyStorePath(Context context) {
		String keyStorePath = new File(context.getFilesDir(), "drozer.bks").getAbsolutePath();
		return this.getSettings().getString(Server.SERVER_KEYSTORE_PATH, keyStorePath);
	}
	...
	public void load(Context context, Server server) {
		this.server = server;
		...
		if(this.isSSL()) {
			server.setSSL(this.isSSL());
			server.setKeyStorePath(this.getKeyStorePath(this.context));
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
		...
		if(key.equals(Server.SERVER_SSL)) {
			...
			server.setKeyStorePath(this.getKeyStorePath(context));
			server.setKeyStorePassword(this.getKeyStorePassword());
			server.setKeyPassword(this.getKeyPassword());
		}
